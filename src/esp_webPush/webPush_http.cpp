...
WebPushResult ESPWebPush::sendPushRequest(const std::string &endpoint,
    ...
    printHeaderErr(esp_http_client_set_header(client, "Authorization", authHeader.c_str()), "Authorization");
    printHeaderErr(esp_http_client_set_header(client, "TTL", ttlValue.c_str()), "TTL");
    printHeaderErr(esp_http_client_set_header(client, "Content-Encoding", "aesgcm"), "Content-Encoding");
    printHeaderErr(esp_http_client_set_header(client, "Content-Type", "application/octet-stream"), "Content-Type");
    printHeaderErr(esp_http_client_set_header(client, "Encryption", encryptionHeader.c_str()), "Encryption");
    printHeaderErr(esp_http_client_set_header(client, "Crypto-Key", cryptoKeyHeader.c_str()), "Crypto-Key");
